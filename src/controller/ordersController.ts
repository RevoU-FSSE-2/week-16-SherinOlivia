...
const createNewOrder = async (req: Request, res: Response) => {
    try {
        ...
        if (role == "staff" || role == "admin") {
            ...
            const [newOrder] = await DBLocal.promise().query(`INSERT INTO railway.orders (custId, product_name, order_qty, total, status, order_datetime, isDeleted)
            ...
            const [createdOrder] = await DBLocal.promise().query(`SELECT * FROM railway.orders WHERE id = ?`,
...
const updateOrder = async (req: Request, res: Response) => {
    ...
    try {
        const getOrder = await DBLocal.promise().query(`SELECT * FROM railway.orders WHERE id = ? AND isDeleted = ?`,
        ...
        if (getOrder[0].length > 0) {
            ...
            return
                ...
                await DBLocal.promise().query(`UPDATE railway.orders SET status = ? WHERE id = ?`,
                [status, id])
    
                const updatedOrder = await DBLocal.promise().query(`SELECT * FROM railway.orders WHERE id = ?`,
                [id]) as RowDataPacket[]
